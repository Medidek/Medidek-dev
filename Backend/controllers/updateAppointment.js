...
const updateAppointment = async function (req, res) {
    try {
        ...
        patient_appointment
        SET name = '${name}', age = '${age}', gender = '${gender}',  phone = '${phone}',  status = '${appointment_status}',  name = '${opd_status}', 
        ...
        //     status = CASE WHEN '${appointment_status}' IS NOT NULL THEN '${appointment_status}' ELSE status END,
        ...
        if (appointmentData.length === 1) {
            return res.status(200).send({ status: true, message: "Appointments updated successfully", data: appointmentData[0] });
        } else {
            return res.status(200).send({ status: true, message: "Appointments updated successfully", data: appointmentData });
        }
